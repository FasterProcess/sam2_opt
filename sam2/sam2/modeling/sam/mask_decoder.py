...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        if repeat_image:
            src = torch.repeat_interleave(
                image_embeddings, tokens.shape[0] // image_embeddings.shape[0], dim=0
            )
        else:
        ...
        #     image_embeddings, tokens.shape[0] // image_embeddings.shape[0], dim=0
        # )
        assert src.shape[0] == tokens.shape[0]
        ...
        return self.inference_predict_masks(
    ...
    def inference_predict_masks_torch(
        ...
        if not self.use_high_res_features:
            ...
            upscaled_embedding = act1(ln1(dc1(src) + high_res_feature1))
            upscaled_embedding = act2(dc2(upscaled_embedding) + high_res_feature0)

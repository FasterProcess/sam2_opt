...
def export_prompt_encoder(
    onnx_name="video_prompt_encoder.onnx", simplify_onnx=True, override=False
):
    """
    ...
    points_coords_base = torch.randint(
        0, 1024, (1, 2, 2), dtype=torch.float, device=device
    )
    ...
    print(
        f"    Base output shapes: sparse_emb={base_sparse_emb.shape}, dense_emb={base_dense_emb.shape}"
    )

    # Scenario 2: Variant input with 5 points
    print("  - Running with variant input (N=1, 5 points)...")
    points_coords_variant = torch.randint(
        0, 1024, (1, 5, 2), dtype=torch.float, device=device
    )
    points_labels_variant = torch.tensor(
        [[1, 0, 1, 0, 1]], dtype=torch.int, device=device
    )
    ...
    print(
        f"    Variant output shapes: sparse_emb={variant_sparse_emb.shape}, dense_emb={variant_dense_emb.shape}"
    )

    # Assertions: Sparse embeddings should change with num_points, dense should not.
    assert (
        base_sparse_emb.shape[1] != variant_sparse_emb.shape[1]
    ), "Sparse embeddings token dimension should be dynamic with num_points."
    assert (
        base_dense_emb.shape == variant_dense_emb.shape
    ), "Dense embeddings shape should be static regardless of num_points."
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            ...
            "sparse_embeddings": {
                0: "N",
                1: "num_tokens",
            },  # Making token dimension dynamic
...
def export_memory_encoder(
    onnx_name="video_memory_encoder.onnx", simplify_onnx=True, override=False
):
    """
    ...
    try:
        ...
        for i, (base_shape, var_shape) in enumerate(zip(base_shapes, variant_shapes)):
            assert (
                var_shape[0] == 3
            ), f"Output {i} batch dim should be 3, but got {var_shape[0]}"
            assert (
                base_shape[1:] == var_shape[1:]
            ), f"Output {i} non-batch dims should be static. Base: {base_shape[1:]}, Variant: {var_shape[1:]}"
        ...
        if simplify_onnx:
            simplify_and_save(
                onnx.load(save_path), save_path.replace(".onnx", "_opt.onnx")
            )
